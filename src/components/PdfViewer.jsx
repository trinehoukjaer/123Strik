...
export default function PdfViewer({ pdfUrl, currentPage, onPageChange }) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <div className="no-print flex items-center gap-4 mb-4">
        <button
          ...
          className="px-4 py-2 rounded-lg bg-nordic-200 text-nordic-800 font-semibold
                     disabled:opacity-40 hover:bg-nordic-300 transition-colors text-lg"
        >
          &larr; Forrige
        </button>
      ...
      {/* PDF document */}
      <div className="border border-warm-200 rounded-xl overflow-hidden shadow-md bg-white max-w-full">
        <Document
          file={pdfUrl}
          onLoadSuccess={onDocumentLoadSuccess}
          loading={
            <div className="flex items-center justify-center h-64 text-nordic-500">
              Indlæser PDF...
            </div>
          }
          error={
